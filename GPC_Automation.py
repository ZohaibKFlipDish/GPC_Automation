...
async def wait_for_element(page, selector, timeout=30000):
    try:
        ...
        st.error(f"Timeout waiting for element {selector}")
        raise
...
async def click_button_by_material_radio_group_debug_id(page, debug_id, index=0):
    selector = f"material-radio-group[debug-id='{debug_id}'] input[type='radio']"
    radio_buttons = await page.query_selector_all(selector)
    if len(radio_buttons) > index:
        await radio_buttons[index].click()
        print(f"Radio button {index + 1} clicked.")
    else:
        print(f"No radio button found at index {index}.")
...
async def click_button_by_xpath(page, xpath):
    try:
        ...
        element = await page.wait_for_selector(f'xpath={xpath}', timeout=30000)
...
async def click_checkbox_by_debug_id(page, debug_id, index=0):
    try:
        checkboxes = await page.query_selector_all(f"material-checkbox[debug-id='{debug_id}']")
...
async def wait_for_login(page):
    st.info("Waiting for login... Please log in manually.")
    while True:
        try:
            await page.wait_for_selector("#main-content", timeout=3000)
            st.success("Login detected, proceeding with automation...")
            break
        except:
...
async def automate_play_console(app_names):
    async with async_playwright() as p:
        ...
        app_list_url = "https://play.google.com/console/u/0/developers/8453266419614197800/create-new-app"
...
PAYMENT METHODS: screen has been designed to show information, it is not possible to modify, add or remove payment methods like Cash etc. This can only be done for payment cards. BANK CONTACT &IDEAL only for NL stores.""")
                ...
                try:
                    ...
                    print(f"Failed to click the radio button: {e}")
                ...
                # Upload CSV
                file_upload_xpath = "/html/body/div[2]/div[3]/div/focus-trap/div[2]/relative-popup/div/span/div/div[2]/console-block-1-column/div/div/console-file-uploader/div[1]/div/div[2]/div/button/i"
                file_path = "/Users/zohaibkhan/GPC_Automation/data_safety_export_Jan24.csv"

                try:
                    upload_file(page, file_upload_xpath, file_path)
                except Exception as e:
                    print(f"Error during file upload: {e}")

                # Import button
                try:
                    await click_button_by_xpath(page, "/html/body/div[2]/div[3]/div/focus-trap/div[2]/relative-popup/div/span/div/div[2]/form-bottom-bar/bottom-bar-base/div/div/div/div[2]/console-button-set/div/button[1]/span")
                    print("Button clicked successfully.")
                except Exception as e:
                    print(f"Failed to click the button: {e}")
                ...
                # Import button
                try:
                    await click_button_by_xpath(page, "/html/body/div[2]/div[4]/material-dialog/focus-trap/div[2]/div/footer/div/div/console-button-set/div/button[2]/span")
                except Exception as e:
                    print(f"Failed to click the element: {e}")
                ...
                # Health apps button
                try:
                    await click_button_by_xpath(page, "/html/body/div[1]/root/console-chrome/div/div/div/div[1]/div/div[1]/page-router-outlet/page-wrapper/div/app-dashboard-page/console-section[2]/div/div/console-block-1-column/div/div/setup-goal/goal/div/div[2]/expandable-area/expandable-container/div/div/div/div/task-group[1]/div[2]/div/task[10]/div/div[2]/div/material-icon/i")
                except Exception as e:
                    print(f"Failed to click the element: {e}")
                ...
                # Cross button
                try:
                    xpath = "/html/body/div[2]/div[2]/div/focus-trap/div[2]/relative-popup/div/span/div/div[1]/div/button/i"
                    await click_button_by_xpath(page, xpath)
                    print("Button clicked successfully.")
                except Exception as e:
                    print(f"Failed to click the button: {e}")
                ...
                # Edit button
                try:
                    xpath = "/html/body/div[1]/root/console-chrome/div/div/div/div[1]/div/div[1]/page-router-outlet/page-wrapper/div/store-settings-page/console-form/console-section[2]/div/console-header/div/div/div[1]/div[2]/div/console-button-set/div/material-button"
                    await click_button_by_xpath(page, xpath)
                except Exception as e:
                    print(f"Failed to click the button: {e}")
                ...
                try:
                    text_field = await wait_for_element(page, By.XPATH, "//*[@id='default-acx-overlay-container']/div[4]/div/focus-trap/div[2]/relative-popup/div/span/div/div[2]/console-block-1-column/div/div/console-form-row[1]/div/div[2]/div[1]/material-input/label/input")
                    await text_field.fill("")  # Clear the field
                    await text_field.type("help@flipdish.com")
                ...
                # Save button
                try:
                    xpath = "//*[@id='default-acx-overlay-container']/div[4]/div/focus-trap/div[2]/relative-popup/div/span/div/div[2]/publishing-bottom-bar/form-bottom-bar/bottom-bar-base/div/div/div/div[2]/console-button-set/div[2]/overflowable-item[2]/button/span"
                    await click_button_by_xpath(page, xpath)
                except Exception as e:
                    print(f"Failed to click the button: {e}")
                ...
                # Cross button
                try:
                    xpath = "//*[@id='default-acx-overlay-container']/div[4]/div/focus-trap/div[2]/relative-popup/div/span/div/div[1]/div/button/i"
                    await click_button_by_xpath(page, xpath)
                    print("Button clicked successfully.")
                except Exception as e:
                    print(f"Failed to click the button: {e}")
                ...
                try:
                    xpath = "/html/body/div[1]/root/console-chrome/div/material-drawer[2]/navigation/nav/div/div[6]/navigation-item/div/expandable-container/div/div/navigation-item[1]/div/expandable-container/div/div/navigation-item[1]/div/a/span"
                    await click_button_by_xpath(page, xpath)
                    print("Navigation item clicked successfully.")
                except Exception as e:
                    print(f"Failed to click the navigation item: {e}")

                st.write(f"Processed: {app_name}")
            except Exception as e:
                st.error(f"Error for '{app_name}': {e}")

        await browser.close()

# Streamlit UI
st.title("Play Console Automation")

app_names_input = st.text_area("Enter app names (one per line):")

if st.button("Run Automation"):
    if app_names_input.strip():
        app_names = [name.strip() for name in app_names_input.strip().split("\n") if name.strip()]
        asyncio.run(automate_play_console(app_names))
    else:
        st.warning("Please enter at least one app name.")
